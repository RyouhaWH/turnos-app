...
export const RightPanel = memo(({
    ...
    return (
        <div className={`${isMobile ? 'mt-0 mb-0 absolute left-0 bottom-0 w-full bg-slate-50/95 backdrop-blur-sm border-t border-slate-200 dark:border-slate-700 transition-all duration-300 ease-in-out flex flex-col gap-0.5 z-10' : 'flex flex-col gap-4 xl:w-[320px]'}`}>
            ...
            <Card className={`${isMobile ? 'border-0 !:mb-0 pb-0 bg-white/95 dark:bg-slate-900/95 shadow-none max-h-none transition-all duration-300 ease-in-out hover:bg-slate-50 dark:hover:bg-slate-800/50' : 'max-h-[40.5vh] overflow-clip border-slate-200/50 shadow-xl backdrop-blur-sm dark:bg-slate-900/90'}`}>
                <CardHeader
                    className={`cursor-pointer pb-2 transition-colors ${isMobile ? 'border-b border-slate-200 dark:border-slate-700 hover:bg-slate-100/50 dark:hover:bg-slate-700/50 py-3' : 'border-b border-slate-100 hover:bg-slate-100/50 dark:border-slate-800 dark:bg-slate-800/50 dark:hover:bg-slate-700/50'}`}
